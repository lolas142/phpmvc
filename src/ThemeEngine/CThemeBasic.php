...
class CThemeBasic implements IThemeEngine, \Anax\DI\IInjectionAware
{
    ...
    public function setSiteTagline($value)
    {
        return $this->setData('siteTagline', $value);
    }
    public function setSiteTitle($value)
    {
        return $this->setData('siteTitle', $value);
    }
    public function setData($which, $value)
    {
        $this->config['views'][0]['data'][$which] = $value;
        //die(var_dump($this->config['views'][0]));
        return $this;
    }
    ...
    {
        $this->di->logger->Timestamp(__CLASS__, __METHOD__, 'render starts');
        // Prepare details
        ...
        $view->setDI($this->di);
        $view->render();
        $this->di->logger->Timestamp(__CLASS__, __METHOD__, 'render ends');
    }
}
